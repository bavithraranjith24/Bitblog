...
const corsOptions: CorsOptions = {
  origin(orgin, callback) {
    if (
      ...
      //Reject requestes from non-whitelisted WHITELIST_ORIGINS
      callback(
...
//Enable response compression to resuce payload size and improve performance
app.use(
  compression({
...
/**
 * Imediately Invoked Async Function Expression (IIFE) to start the server.
 ...
 *
 * -Attemts to disconnect from the database before shutting down the server.
 * -Logs a success if the disconnecting is successful.
 ...
 *
 * -'SIGTERM' is typically sent when stopping a process (e.g., ' kill'  commend or container shutdown).
